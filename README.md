...
NyaMarkdownor is a local-first Markdown desktop editor for Windows, macOS, and Linux. Version `1.0.5` provides source, split, preview, and visual editing; multi-document local workflows; structured table tools; Markdown-aware clipboard behavior; and guarded file recovery/version history without requiring an account or remote service.
...
See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the current architecture and [docs/RECOVERY_AND_VERSION_HISTORY.md](docs/RECOVERY_AND_VERSION_HISTORY.md) for the Chinese recovery/version-history specification.

## Downloads

Installers for Windows, macOS, and Linux are published on [GitHub Releases](https://github.com/stevennight/NyaMarkdownor/releases/latest), together with `SHA256SUMS` for verification.
...
The packaged app reports its version, commit, UTC build time, and release repository in **Settings > About**. Local builds use the development version derived from `package.json` (for example, `1.0.5-dev`).
...
- The interface supports English and Simplified Chinese, with an explicit language setting and a system-language mode.
...
- Multi-tab editing: new local files, local draft tabs, and opened files live in document tabs, so workspace/recent-file navigation no longer has to replace the current editor buffer. The primary New File action creates a real saved Markdown file through a create-new-only local path instead of silently making an unbound draft or overwriting an existing file; a separate New Draft Tab command remains available for temporary scratch work. The Open dialog supports selecting multiple Markdown/text files at once, deduplicates selected paths with Windows slash/case normalization, opens the readable files as tabs, and reports partial failures instead of dropping the whole batch. Opening a real file from a clean blank or bundled-sample draft reuses that placeholder instead of leaving a stray Untitled tab behind. The full open-tab session is restored on restart, dirty tabs are marked inline, closing a dirty tab first creates a required safety checkpoint, and batch close commands can close other/right/saved/all tabs safely. Each tab keeps isolated runtime editor state so undo/redo history does not leak across documents; keyboard navigation supports next/previous/direct tab switching; active tabs scroll into view when selected from keyboard or commands; and tabs can be reordered by drag or command-palette actions. Recovery and command paths track which tab owns the currently mounted CodeMirror view, so last-moment text cannot be merged into the wrong tab during rapid tab switches.
...
- Toolbar and command-palette copy actions honor CodeMirror multi-selection and use the same Markdown-aware clipboard payloads as `Ctrl+C`; source Markdown copy preserves canonical spacing, while compact Markdown copy removes non-literal block-separator blank lines for plain external paste without changing the document. Smart `Ctrl+X` cuts ordinary selections with the same clean text/HTML/Markdown payload before deleting the selected Markdown source. Selected table columns and rectangular cell ranges copy consistently without depending on browser DOM selection quirks.
...
- Safe local saves combine disk-change detection with unified version history. Ordinary automatic and `Ctrl+S` saves share one rolling automatic version per source and seal it on the configured interval (10 minutes by default). Destructive replacements create fixed safety checkpoints, while user-created checkpoints use an independently protected manual pool. New disk versions live under the Tauri app-local-data directory, or a backup directory chosen in Settings, instead of adding a folder beside each document. The configurable defaults retain 48 automatic, 32 safety, and 32 manual versions per source, with a shared limit of 2,048 entries or 2 GiB, a 256-MiB maximum backup candidate, 180-day automatic retention, 365-day safety retention, unlimited-by-age manual retention, and a 365-day orphan grace period. Manual versions are never silently evicted. Existing sibling `.nyamarkdownor-backups` folders remain readable as legacy history but receive no new files. Real external content changes or unverifiable paths are left for review instead of being overwritten.
...
- Work recovery and version history are separate. Work recovery continuously mirrors the open-tab session, active draft, preferences, recent files, and workspace root into native app-data JSON records so restart can resume the latest working state; normal window close flushes that state and does not create a history version. Fixed local safety checkpoints protect dirty editor content before closing a tab, reloading, restoring, discarding recovered content, or resolving a save conflict; overwriting an existing Save As target protects that target's disk content as a fixed disk safety version. Manual checkpoints are user-created. The unified Version History manager groups disk versions and local checkpoints by document, separates confirmed orphaned histories, supports per-version or per-document deletion, and can open any retained entry as an independent draft. Versions up to 16 MiB can be compared interactively with the current Markdown source; larger versions remain restorable without running a main-thread diff.
...
## Data Locations And Application Identity

The packaged application identifiers are `io.github.stevennight.nyamarkdownor` for release builds and `io.github.stevennight.nyamarkdownor.dev` for development builds. Tauri derives the native application data and local-data directories from those identifiers, keeping installed and development state isolated. NyaMarkdownor does not scan or migrate data from earlier bundle identifiers.

Application state (preferences, tabs, drafts, checkpoints, recent files, and workspace state) uses Tauri's app-data directory. Central disk version history uses `backups-v1` under Tauri's app-local-data directory unless the user chooses another backup root in Settings. Browser preview state remains development-only and does not represent path-backed desktop files.
